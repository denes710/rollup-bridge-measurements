...
describe("Tests for gas measurements", function () {
    ...
    it("L1<>L2 communication", async function () {
        ...
        rawTx = await optimismSpokeConnector.connect(helperOptimismAmbSigner).populateTransaction.processMessage(NON_NULL_BYTES32);
        ...
        rawTx = await optimismSpokeConnector.connect(owner).populateTransaction.proveAndProcess(proofs, NON_NULL_BYTES32, aggregateProof, NON_ZERO_BYTE_UINT256);
    ...
    it("X communication", async function () {
        const [owner, sender, user, remoteUser, fake] = await ethers.getSigners();
        const ZERO_ADDRESS = '0x0000000000000000000000000000000000000000';
        const NON_NULL_BYTES32 = '0x1111111111111111111111111111111111111111111111111111111111111111';
        ...
        const inputEncodedData = await optimismGasHelper.encodeCallData(999, NON_ZERO_BYTE_ADDR, NON_ZERO_BYTE_ADDR, NON_ZERO_BYTE_UINT256, "");

        console.log("input encoded data len: " + inputEncodedData.length);
        ...
        await bridgeFacet.connect(owner)['xcall(uint32,address,address,address,uint256,uint256,bytes)'](999, remoteUser.address, ZERO_ADDRESS, fake.address, 0, 10000, inputEncodedData);
    });
    it("Bytes measurement X call", async function () {
        const [owner, sender, user, remoteUser, fake] = await ethers.getSigners();
        const ZERO_ADDRESS = '0x0000000000000000000000000000000000000000';
        const NON_NULL_BYTES32 = '0x1111111111111111111111111111111111111111111111111111111111111111';
    ...
    it("Bytes prove and processs", async function () {
        ...
        for (let i = 1; i < 2**16; i = i * 2) {
            ...
            console.log("Proofs: " + i + " \t Consumed gas: " + result + " \t \t "+ " \t L1 fee(GWei): " + estimatedL1Gas/10**9);
        }
    });
});