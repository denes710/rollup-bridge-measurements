...
contract Hashi721Bridge is IHashi721Bridge, ERC721HolderUpgradeable, HashiConnextAdapter {
  ...
  function setForWrapper(address _asset, uint32 _originalDomainId, address _originalAsset) public  {
    originalDomainIds[_asset] = _originalDomainId;
    originalAssets[_asset] = _originalAsset;
  }
  ...
  function xCall(
    ...
    if (originalAssets[asset] == address(0x0) && originalDomainIds[asset] == 0) {
      ...
      originalDomainId = uint32(ConnextHelper(connext).domain());
      originalAsset = asset;
    } else {
  ...
  function _xReceive(bytes memory callData) internal override {
    ...
    if (originalDomainId != uint32(ConnextHelper(connext).domain())) {
