...
describe("Tests for gas measurements", function () {
    it("Wrapping cycle", async function () {
        ...
        for (let i = 1; i < 10; i++) {
            await simpleGatewayDstSpokeBrdige.connect(relayer).minting(i - 1, remoteUser.address, i, wrappedERC721.address);
        }
        ...
        for (let i = 1; i < 10; i++) {
            ...
            await simpleGatewayDstSpokeBrdige.connect(remoteUser).createBid(user.address, i, wrappedERC721.address, i - 1, {value: ethers.utils.parseEther("0.01")});
        }
        for (let i = 1; i < 10; i++) {
            await simpleGatewayDstSpokeBrdige.connect(relayer).buyBid(i - 1);
        }
        for (let i = 1; i < 10; i++) {
            await simpleGatewaySrcSpokeBrdige.connect(relayer).unlocking(i - 1, i - 1, user.address);
        }

        await time.increase(3600 * 4);

        for (let i = 1; i < 10; i++) {
            await simpleGatewaySrcSpokeBrdige.connect(user).claimNFT(i - 1);
        }
    });
    it("Challing cycle", async function () {
        ...
        for (let i = 1; i < 10; i++) {
            await simpleGatewayDstSpokeBrdige.connect(relayer).minting(i - 1, relayer.address, i, wrappedERC721.address);
        }        await simpleGatewayDstSpokeBrdige.connect(wathcer).challengeMinting(2, {value: ethers.utils.parseEther("10.00")});
